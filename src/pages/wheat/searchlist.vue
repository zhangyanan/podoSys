<template>
  <f7-page>
    <div class="list media-list">
      <ul>
        <div v-for="(item, index) in this.ret_searchList.datas" :key="index">
          <li>
             <a href = "javascript:;" class="item-link item-content" @click="local_setSelectedGG(item.键值)">
...
export default {
  ...
  created () {
    //得到当前用户查询的gg列表
    //var whatdata = sessionStorage.getItem('g_searchList')
    //console.log(whatdata)
    //this.ret_searchList = JSON.parse(sessionStorage.getItem('g_searchList'))
    //this.getPersonalAccount('cui')
    //console.log(this.l_ret_search_data.datas[0].姓名)

    this.ret_searchList = this.l_ret_search_data
  },
  methods: {
